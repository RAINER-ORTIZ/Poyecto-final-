...
const cotizacion = () => {
  const edad = parseInt(prompt('¿Que edad tiene?'));

  if (edad < 18) {
    return alert('No califica para la cotizacion');
  }
  ...
  const sueldo = parseInt(prompt('¿cuanto gana al mes?'));

  recargo += sueldo * 0.05;

  const pareja = prompt('¿Esta casado?');
